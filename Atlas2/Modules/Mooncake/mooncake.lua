...
function M:write_str(node)
    ...
    local function quote_string(str)
        -- Only quote string if it does not contain any quotes already
    ...
    dispatch_node = function (node)
        ...
        -- Special handling for strings - surround them with '',
        -- since otherwise empty strings are treated as nil on load.
        elseif (type(node) == 'string') then
            return handle_scalar(quote_string(node))
        else
